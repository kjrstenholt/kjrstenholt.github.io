...
CourseCmi5Plugin.prototype.fail = function (userScoreObj) {
    ...
    if (this._shouldSendStatement()) {
        return this.cmi5.failed(userScoreObj).then(() => {}, err => {console.log(err.message);});
    }
    return Promise.resolve(null);
}
...
CourseCmi5Plugin.prototype.passAndComplete = function (userScoreObj) {
    ...
    if (this._shouldSendStatement()) {
        return this._sendStatementViaLibFunction(() => {
            return this.cmi5.passed(userScoreObj).then(() => this.cmi5.completed(), err => {
 ...
 *
 * @param  {string} value           What the learner experienced
 * @return {Promise}                The promise from the statement network call.
 */


CourseCmi5Plugin.prototype.experienced = function (value) {
    if (!this.cmi5) {
        return Promise.resolve(null);
    }
    ...
    stmt.object = {
        objectType: "Activity",
		id: this.cmi5.getActivityId() + "id_" + String(++counter_id),  //replace this counter id with genuine id
		//id: "id_" + String(++counter_id),  //replace this counter id with genuine id
        definition: {
            name: {"en-US": value},
        }
    };
 ...
 *
 * @param  {Objec
 t} statement A cmi5 statement to save.
 * @return {Promise}          The network call attempting to save the cmi5 statement.
 * @private
 */
CourseCmi5Plugin.prototype._sendStatement = function (statement) {
    this.activeStatements += 1;
    //this.callbackOnStatementSend(null, null, this.activeStatements);  //restore this
    return this.cmi5.sendStatement(statement).then(result => {
        ...
        this.activeStatements -= 1;
        if (this.callbackOnStatementSend) {
            this.callbackOnStatementSend(error, error, this.activeStatements);
        }
    })
}
...
CourseCmi5Plugin.prototype.trackingError = function (errorMsg) {
    if(console && console.log){
		console.log("Tracking Error " + errorMsg);
	}
}

CourseCmi5Plugin.prototype.setRawScore = function (value) {
    this.rawScore = value;
}

CourseCmi5Plugin.prototype.getRawScore = function () {
    return this.rawScore;
}

CourseCmi5Plugin.prototype.setMinScore = function (value) {
    this.minScore = value;
}

CourseCmi5Plugin.prototype.getMinScore = function () {
    return this.minScore;
}

CourseCmi5Plugin.prototype.setMaxScore = function (value) {
    this.maxScore = value;
}

CourseCmi5Plugin.prototype.getMaxScore = function () {
    return this.maxScore;
}

CourseCmi5Plugin.prototype.setScaledScore = function (value) {
    this.scaledScore = value;
}

CourseCmi5Plugin.prototype.getScaledScore = function () {
    return this.scaledScore;
}

CourseCmi5Plugin.prototype.setCompletionStatus = function (value) {
    this.completionStatus = value;
}

CourseCmi5Plugin.prototype.getCompletionStatus = function () {
    return this.completionStatus;
}

CourseCmi5Plugin.prototype.setSuccessStatus = function (value) {
    this.successStatus = value;
}

CourseCmi5Plugin.prototype.getSuccessStatus = function () {
    return this.successStatus;
}

CourseCmi5Plugin.prototype.scored = function (value) {
    if (!this.cmi5) {
        return Promise.resolve(null);
    }

    let stmt = this.cmi5.prepareStatement(VERB_SCORED_OBJ.id);
    stmt.verb.display = VERB_SCORED_OBJ.display;
    stmt.object = {
        objectType: "Activity",
		id: this.cmi5.getActivityId() + "id_" + String(++counter_id),  //replace this counter id with genuine id
		//id: "id_" + String(++counter_id),  //replace this counter id with genuine id
...
CourseCmi5Plugin.prototype.evaluate = function () {
    if (!this.cmi5) {
        return Promise.resolve(null);
    }
	
var scoreObj = {
        scaled: this.getScaledScore(),
        raw: this.getRawScore(),
        min: this.getMinScore(),
        max: this.getMaxScore()
    }; 
	
var masteryScore = this.passingScore;

if(Number.isNaN(masteryScore)){
	
	if(this.successStatus == "passed" || this.successStatus == "completed"){
		this.passAndComplete(scoreObj);
	}
	if(this.successStatus == "failed"){
		this.fail(scoreObj);
	}
    
}
else{
	if(this.getScaledScore() >= masteryScore){
		this.passAndComplete(scoreObj);
	}
	else{
		this.fail(scoreObj);
